...
The executable still requires the Claude Code CLI to be installed and available as `claude` on the user's PATH.
...
GitHub Actions will build the Windows executable and attach `AutoGenerateDaltonArticles-windows.zip` to the GitHub release.

You can also run the workflow manually from the GitHub Actions tab. Manual runs upload the zip as a workflow artifact, but tagged pushes create a GitHub release.
...
- `build_exe.ps1`: local PyInstaller build script
- `.github/workflows/release.yml`: GitHub Actions release build
